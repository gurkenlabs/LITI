...
public abstract class Player extends Creature implements IUpdateable, IRenderable {
  ...
  protected Player(PlayerConfiguration config) {
    ...
    this.onDeath((entity, hitEvent) -> handleDeath(entity));
    ...
    this.addEntityRenderListener(new EntityRenderListener() {
      @Override public void rendered(EntityRenderEvent event) {
      }

      @Override public void rendering(EntityRenderEvent event) {
        if (isDead() || isFalling()) {
          return;
        }
  ...
  @Override public void update() {
    ...
    if (Objects.equals(this.traits().stamina().getValue(), this.traits().stamina().getMin())) {
  ...
  public boolean isStaminaDepleted() {
    return this.staminaDepleted != 0 && Game.time().since(this.staminaDepleted) < Timings.STAMINA_DEPLETION_DELAY;
  }
  ...
  public void setBlocking(boolean blocking) {
    ...
    if (blocking && (this.traits().stamina().getValue() < 0.20 * this.traits().stamina().getMax() || this.isStaminaDepleted())) {
      return;
    }

    if (!this.isBlocking && (this.lastBlock != 0 && Game.time().since(this.lastBlock) < Timings.BLOCK_COOLDOWN)) {
      return;
    }
    ...
    if (this.isBlocking) {
      ...
      this.traits().stamina().setValue(this.traits().stamina().getValue() - this.traits().stamina().getMax() * 0.2);
    } else {
  ...
  @Action(name = "DASH") public void useDash() {
  ...
  @Action(name = "BASH") public void useBash() {
  ...
  @Action(name = "SURVIVALSKILL") public void useSurvivalSkill() {
  ...
  @Override public void updateAnimationController() {
  ...
  @Override protected IEntityAnimationController<Player> createAnimationController() {
  ...
  private void recoverStamina() {
    if (this.traits().stamina().getValue() < this.traits().stamina().getMax()) {
      double recovery = Math.min(Game.loop().getDeltaTime(), GameLoop.TICK_DELTATIME_LAG) * 0.02F * this.traits().recovery().getValue() * Game.loop()
          .getTimeScale();
      if (this.traits().stamina().getValue() + recovery > this.traits().stamina().getMax()) {
        this.traits().stamina().setValue(traits().stamina().getModifiedMax());
      } else {
        this.traits().stamina().setValue(this.traits().stamina().getValue() + recovery);
      }
    }
  }
  ...
  private void recoverHealth() {
    ...
    if (this.getHitPoints().getValue() < this.getHitPoints().getMax()) {
      int recovery = 1;
      if (this.getHitPoints().getValue() + recovery > this.getHitPoints().getMax()) {
        this.getHitPoints().setValue(getHitPoints().getModifiedMax());
      } else {
        this.getHitPoints().setValue(this.getHitPoints().getValue() + recovery);
      }
    }
  }

  private void drainStaminaWhileBlocking() {
    if (this.traits().stamina().getValue() > this.traits().stamina().getMin()) {
      double drain =
          Math.min(Game.loop().getDeltaTime(), GameLoop.TICK_DELTATIME_LAG) * 0.02F * (1.5 - this.traits().recovery().getValue()) * Game.loop()
              .getTimeScale();
      if (this.traits().stamina().getValue() - drain <= this.traits().stamina().getMin()) {
        this.traits().stamina().setValue(traits().stamina().getModifiedMin());
      } else {
        this.traits().stamina().setValue(this.traits().stamina().getValue() - drain);
      }
    }
  }
  ...
  private void handleResurrect(ICombatEntity entity) {
    this.traits().stamina().setValue(traits().stamina().getMax());
